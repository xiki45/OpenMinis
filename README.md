...
上游 `main` 之上 8 个提交、13 个文件（+615/−4）。全部为新增文件与少量插入式改动，
不触碰上游既有功能。
...
| `AssistSession.kt` | 核心：`onHandleAssist` 采集屏幕上下文 → 深链打开 Minis 聊天并注入 |
...
- 深链 `minis://assist`：`DeepLinkHandler` 路由 + `DeepLinkCoordinator.pendingAssist` 暂存，
  `ChatScreen` 首次 compose 时消费 → **屏幕上下文成为发给 agent 的第一条消息**
...
手势 / 按键 / ACTION_ASSIST
   │
   ▼
系统 → AssistSession.onHandleAssist(当前屏幕)
   │  AssistContext.flatten() → 结构化文本
   │  ① DeepLinkCoordinator.pendingAssist
   │  ② startActivity(minis://assist)
   ▼
MainActivity → 新 chat 会话 → ChatViewModel.sendMessage(屏幕上下文)
   ▼
agent 循环正常处理（shell / 技能 / 无障碍读屏…）
```
...
**HyperOS（小米/红米）注意**：手势派发被硬编码到小爱私有通道，改默认助理无效。
...
改动集中在 13 个文件、以新增为主，rebase 冲突概率低。
